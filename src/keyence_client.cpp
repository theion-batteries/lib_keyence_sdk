...
wgm_feedbacks::enum_hw_feedback keyence_client::connect()
{
    ...
    std::cout << "connecting to keyence keyence controller via tcp" << "\n";
    std::cout << "keyence server ip:  " << IP << "\n";
    ...
    std::cout << "open connection to " << IP << "\n";
    RC Conn = LKIF2_OpenDeviceETHER(&paramEther);
    if (Conn == RC_OK)
    {
        std::cout << "keyence connection success" << "\n";
        return wgm_feedbacks::enum_hw_feedback::hw_success;
    }
    else
    {
        std::cout << "keyence connection failed due to " << rc_error[Conn] <<"\n";
        std::string v = rc_error[Conn];
        std::cout<<v<<"\n";
...
double keyence_client::get_value_output(int outputNr)
{
    ...
    if (DataIsValid())
    {
        std::cout << "get valid data from output number: " << outputNr << " Value:" << value.Value << "\n";
        return value.Value;
    }
//    std::cout << "invalid datafrom output number: " << outputNr << " Value:" << value.Value << "\n";
    return 0;
}
...
double keyence_client::get_value_all()
{
    ...
    if (DataIsValid())
    {
        std::cout << "get valid data from output number: " << all << " Value:" << value.Value << "\n";
        return value.Value;
    }
    std::cout << "invalid datafrom output number: " << all << " Value:" << value.Value << "\n";
    return NULL;
}
